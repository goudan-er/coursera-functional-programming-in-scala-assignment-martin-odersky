...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new NoSuchElementException("mostRetweeted of EmptySet")
...
//    ((left union right) union that) incl elem
  ...
  def mostRetweeted: Tweet = {
    if (left.isEmpty && right.isEmpty) elem
      ...
      elem max right.mostRetweeted
    } else if (right.isEmpty) {
      elem max left.mostRetweeted
    } else {
      elem max (left.mostRetweeted max right.mostRetweeted)
    }
  }

  def descendingByRetweet: TweetList = new Cons(mostRetweeted, (this remove elem).descendingByRetweet)
